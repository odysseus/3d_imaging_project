...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    images = [f for f in listdir(label_dir) if (
    ...
    for f in images:
        ...
        image = nib.load(join(image_dir, f)).get_fdata()
        label = nib.load(join(label_dir, f)).get_fdata()
        ...
        # ANSWER: By default the label array will be floating point (dtype('float64'))
